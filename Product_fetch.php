...
if (isset($_POST['search']['value'])) {
    ...
    $query .= " OR Reg_date like '%".$search_value."%' ";
...
$run_result =mysqli_query($conn,$query);
    ...
    while ($row = mysqli_fetch_assoc($run_result)) {
        $sub_data = array();        
        $sub_data[] = $row['id'];
        $sub_data[] = $row['cat_name'];
        $sub_data[] = $row['name'];
        $sub_data[] = $row['description'];
        if ($row['image'] != "") {
        $image = '<img src="'.$row['image'].'" class="avatar avatar-image"  style="width: 80%; object-fit:cover;object-position: top;" />';
        }
        else {
        $image = '';
        }
